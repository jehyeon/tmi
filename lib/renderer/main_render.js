...
function loadToNavigation () {
  ...
  // Add event to button
  DOCUMENT.addCapsuleButton.addEventListener('click', addCapsuleEvent());
  ...
  for (const capsuleName of Object.keys(capsules)) {
    ...
    capsuleElement.querySelector('h3').addEventListener('click', deleteCapsuleEvent(capsuleName, capsuleElement));
    const targets = capsuleElement.querySelectorAll("li.target[exist='true']")
    for (const target of targets) {
      target.addEventListener('click', loadTrainings(capsules[capsuleName][target.innerText].verified));
    };
  }
}

function loadTrainingInfos (trainingInfos) {
  // Reset training infos document
...
function loadTrainings (verifiedTrianings) {
  // Load training infos
  loadTrainingInfos ({ count: verifiedTrianings.lenght });

  // Reset training document
  DOCUMENT.trainings.innerHTML = '';

  // verified 로드
  verifiedTrianings.map(tr => {
    const trainingElement = document.createElement('li');
    trainingElement.innerText = tr.uttr;
...
// 4. Event Functions
function deleteCapsuleEvent (capsuleName, capsuleElement) {
  ...
  DOCUMENT.capsulesElement.removeChild(capsuleElement);
}
...
function addCapsuleEvent (e) {
  ...
  // ! argument 1인 경우도 추가하면 좋을 듯 -> getTrainingsFromStore (capsulePath)
  const targets = getTrainingsFromStore ()[capsuleName];

  // const = Object.keys(targets).map(target => {
  //   name: target,
  //   exist: targets[target].exist
  // })

  DOCUMENT.capsulesElement.appendChild(makeCapsuleElement(capsuleName, ))
}
