...
function deleteMode(e) {
  ...
  if (e.target.getAttribute('on') === 'false') {
    ...
    // ! 모든 항목을 검사하고 있음 / value가 on인 것만 select 가능할 듯 
    const itemToDelete = [...allDeleteBoxes].filter(item => item.checked === true)
      .map(item => item.parentElement.getAttribute('id'));
  
    console.log(itemToDelete);
    // 업데이트 예정
    // DeleteTrainingFromStore(itemToDelete)
