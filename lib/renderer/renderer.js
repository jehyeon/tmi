...
const DOCUMENT = {
  ...
  trainingInfos: document.getElementById('trainingInfos'),
...
function makeTrainingInfoElement(infos) {
  const trainingInfosElement = document.createElement('ul');
  infos.forEach(info => {
    const trainingInfo = document.createElement('li');
    trainingInfo.innerHTML = info;
    trainingInfosElement.appendChild(trainingInfo);
  });
  return trainingInfosElement;
}
...
function clearTrainingInfosElement() {
...
function loadTrainings() {
  ...
  if (STATE.selectedCapsule && STATE.selectedTarget) {
    ...
    // Training infos
    const infos = [`count: ${trainings.length}`];
    DOCUMENT.trainingInfos.appendChild(makeTrainingInfoElement(infos))
