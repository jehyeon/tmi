...
function deleteTrainings() {
  ...
  console.log(store.getCapsuleInfoOf(STATE.selectedCapsule).capsulePath); // path
  console.log(STATE.selectedTarget);  // target
  // item to delete (fileName, traingingId)
  document.querySelectorAll("li.training[checked='true'").forEach(training => {
    console.log(training.getAttribute('id'), training.getAttribute('filename'));
  });

  // store
  // store.deleteTraining(capsule, itemsToDelete);

  // document <- store에서 삭제 후 re-load 고려 안해도 됨

  // local
  // training.deleteTrainings(path, target, itemToDelete);
