...
function modifyToNL (anl) {
  ...
  while (nl.indexOf(')[') != -1) {
    ...
    nl = nl.replace(nl.slice(start, end), '').replace('(', '');
  }
  
  return nl;
}

function getGoalInfo(anl) {
  const goalInfo = {};
  const fullGoal = anl.slice(3, anl.indexOf(']'));
  if (fullGoal.split(',').length > 1) {
    goalInfo['route'] = fullGoal.split(',')[1].slice(2, fullGoal.split(',')[1].length);
  } else {
    // goalInfo['route'] = undefined;
  }

  const splited = fullGoal.split(',')[0].split(':');

  switch (splited.length) {
    case 3:
      goalInfo['goal'] = splited[0];
      goalInfo['spec'] = splited[1];
      goalInfo['node'] = splited[2];
      break;
    case 2:
      goalInfo['goal'] = splited[0];
      goalInfo['spec'] = splited[1];
      goalInfo['node'] = splited[0];
      break;
    case 1:
      goalInfo['goal'] = splited[0];
  }
...
function getValues(anl) {
  const valueInfos = [];
  let raw = anl.slice(anl.indexOf(']') + 1, anl.length).trim();
  while (raw.indexOf('(') !== -1) {
    const fullValue = raw.slice(raw.indexOf('('), raw.indexOf(']') + 1);
    try {
      valueInfos.push({
        value: fullValue.split(')[v:')[0].slice(1,),
        type: fullValue.split(')[v:')[1].slice(0,-1)
      });
    } catch {
      // # need to update
      valueInfos.push({
        value: fullValue.replace(/ /gi, '').split(')[v:')[0].slice(1,),
        type: fullValue.replace(/ /gi, '').split(')[v:')[1].slice(0,-1)
      });
    }
    raw = raw.slice(raw.indexOf(']') + 1, raw.length);
  }
  return valueInfos;
}
...
function get_verify_training_datas (raw_tr_datas) {
  ...
  for (let fileName of Object.keys(raw_tr_datas)) {
    for (let raw_tr_data of raw_tr_datas[fileName]) {
      ...
      const goalInfo = getGoalInfo(tr_uttr);
      const values = getValues(tr_uttr);
...
function deleteTrainings(path, target, trainingsToDeletes) {
  ...
  Object.keys(ItemsToDelete).forEach(fileName => {
    // ! https://github.com/jehyeon/tmi/issues/20
    // path name user customizing or auto find path 
