...
const statusStore = new StatusStore({name: 'Status'});
...
ipcMain.on('get-status', (event) => {
  ...
  event.returnValue = statusStore.getStatus();
});

ipcMain.on('add-status', (event, arg) => {
  if (debugMode) {
    console.log('add-status, ' + JSON.stringify(arg));
  }
  statusStore.addStatus(arg);
});

ipcMain.on('update-status', (event, arg) => {
  if (debugMode) {
    console.log('update-status, ' + JSON.stringify(arg));
  }
  statusStore.updateStatus(arg);
});

ipcMain.on('delete-status', (event, arg) => {
  if (debugMode) {
    console.log('delete-status, ' + JSON.stringify(arg));
  }
  statusStore.deleteStatus(arg);
});
